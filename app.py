...
@app.get("/libros/{id}/valoraciones", response_model=List[dict])
def get_valoraciones_libro(id: int):
    mydb = mysql.connector.connect(host=host_name, port=port_number, user=user_name, password=password_db, database=database_name)
    cursor = mydb.cursor()
    cursor.execute("""
    SELECT v.ID_valoracion, v.ID_libro, v.Valoracion, v.Comentario, v.Fecha
    FROM Valoraciones v 
    WHERE v.ID_libro = %s
    """, (id,))
...
@app.get("/autores/{id}/libros", response_model=List[Libro])
def get_libros_por_autor(id: int):
    mydb = mysql.connector.connect(host=host_name, port=port_number, user=user_name, password=password_db, database=database_name)
    cursor = mydb.cursor()
    cursor.execute("SELECT * FROM Libros WHERE ID_autor = %s", (id,))
    libros = cursor.fetchall()
    cursor.close()
    mydb.close()
